...
const Orders = () => {
  ...
  const filteredOrders = orders.filter((order) => {
    ...
    return order.code.toLowerCase().includes(search) || order.customerName.toLowerCase().includes(search);
  });
  ...
  return (
    <div style={{ padding: '20px', fontFamily: 'sans-serif', backgroundColor: '#f9f9f9', minHeight: '100vh' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px' }}>
        ...
        <button style={{ backgroundColor: '#5a3d28', color: '#fff', border: 'none', padding: '10px 20px', borderRadius: '6px', cursor: 'pointer' }}>+ Yeni Sifariş</button>
      </div>

      <div style={{ marginBottom: '20px', position: 'relative', width: '300px' }}>
        <span style={{ position: 'absolute', left: '10px', top: '50%', transform: 'translateY(-50%)', color: '#000', fontSize: '16px' }}>🔍</span>
        <input 
          type="text" 
          placeholder="Axtarış..." 
          value={searchTerm} 
      ...
      <div style={{ overflowX: 'auto', backgroundColor: '#fff', borderRadius: '8px', boxShadow: '0 2px 5px rgba(0,0,0,0.05)' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '13px' }}>
          <thead>
            <tr style={{ borderBottom: '2px solid #f0f0f0', color: '#555' }}>
              <th style={{ padding: '12px' }}>KOD</th>
              <th style={{ padding: '12px' }}>MÜŞTƏRİ / TEL</th>
              <th style={{ padding: '12px' }}>TARİXLƏR</th>
              <th style={{ padding: '12px' }}>MƏHSUL</th>
              <th style={{ padding: '12px' }}>KAT.</th>
              <th style={{ padding: '12px' }}>İP</th>
              <th style={{ padding: '12px', textAlign: 'center' }}>RƏNG</th>
              <th style={{ padding: '12px' }}>HÖRGÜ / ÖLÇÜ</th>
              <th style={{ padding: '12px' }}>MAYA</th>
              <th style={{ padding: '12px' }}>SATIŞ</th>
          ...
          <tbody>
            {filteredOrders.map((order) => (
              <tr key={order.id} style={{ borderBottom: '1px solid #f9f9f9' }}>
                ...
                <td style={{ padding: '12px' }}><div>{order.customerName}</div><div style={{ fontSize: '11px', color: '#888' }}>{order.customerPhone}</div></td>
                ...
                <td style={{ padding: '12px' }}>{order.pattern}<br/><span style={{ fontSize: '11px', color: '#888' }}>{order.size}</span></td>
                ...
                <td style={{ padding: '12px' }}>
                  <select 
                    ...
                    style={{ padding: '5px', backgroundColor: statusOptions[order.status].bg, color: statusOptions[order.status].color, border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold' }}
                  >
                    {Object.keys(statusOptions).map(s => <option key={s} value={s}>{s}</option>)}
                  </select>
                </td>
                <td style={{ padding: '12px', textAlign: 'center' }}>
                  <button style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '16px', marginRight: '8px' }}>✎</button>
